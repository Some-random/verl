...
log_dir=$PWD/logs/feedback_rl_gpt_oss_20b_lora_${timestamp}
mkdir -p $log_dir
...
# Use OpenAI GPT-OSS-20B model (smaller than 32B)
...
experiment_name=gpt_oss_20b_feedback_rl_lora8
...
# Since 20B is smaller than 32B, we can use larger batch sizes
...
# ================= performance =================
infer_tp=2 # vllm tensor parallel - 20B fits well with tp=2
train_sp=2 # train sequence parallel
offload=True

# With 20B model, we have more memory headroom
