...
log_dir=$PWD/logs/feedback_rl_qwen3_32b_lora_${timestamp}
mkdir -p $log_dir
...
experiment_name=qwen3-32b_feedback_rl_lora8
...
train_batch_size=16  # Reduced further for 32B model
ppo_mini_batch_size=4  # Smaller mini batches
n_resp_per_prompt=2  # Generate 2 responses per prompt
n_resp_per_prompt_val=2  # Reduced from 4
...
# ================= performance =================
infer_tp=4 # vllm tensor parallel - use all 4 GPUs for inference to reduce memory per GPU
train_sp=2 # train sequence parallel
offload=True

# Reduce token length per GPU to fit in memory
