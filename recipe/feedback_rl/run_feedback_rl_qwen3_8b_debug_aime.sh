...
log_dir=$PWD/logs/feedback_rl_qwen3_8b_debug_aime_${timestamp}
mkdir -p $log_dir
...
# DEBUG: Use AIME for both training and validation to test if model can learn
train_files="['aime_2024']"
test_files="['aime_2024']"  # Same as training to check if it can overfit
...
experiment_name=qwen3-8b-sft_debug_aime
...
# 8B model - smaller batches for debugging with small AIME dataset
train_batch_size=8  # Very small since AIME only has 30 problems
ppo_mini_batch_size=4
n_resp_per_prompt=4  # More responses to see variety
n_resp_per_prompt_val=4  # Same for validation

# Validation batch size
val_batch_size=30  # Process all 30 AIME problems at once
