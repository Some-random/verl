...
log_dir=$PWD/logs/feedback_rl_qwen3_8b_debug_math_${timestamp}
mkdir -p $log_dir
...
# DEBUG: Use MATH for both training and validation to test if model can learn
train_files="['math']"
test_files="['math']"  # Same as training to check if it can overfit
...
experiment_name=qwen3-8b-sft_debug_math
...
# 8B model - smaller batches for debugging
train_batch_size=16  # Smaller for faster iterations
ppo_mini_batch_size=4
n_resp_per_prompt=4  # More responses to see variety
n_resp_per_prompt_val=4  # Same for validation

# Validation batch size
val_batch_size=100  # Process 100 MATH problems for validation
