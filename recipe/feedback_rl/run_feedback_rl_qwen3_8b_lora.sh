...
log_dir=$PWD/logs/feedback_rl_qwen3_8b_sft_lora_${timestamp}
mkdir -p $log_dir
...
experiment_name=qwen3-8b-sft_feedback_rl_lora8
...
# 8B model - can use even larger batch sizes due to small model size
train_batch_size=64  # Larger batches for better training
ppo_mini_batch_size=16
n_resp_per_prompt=4  # Generate 4 responses per prompt
...
# With 8B model, we have lots of memory headroom
actor_max_token_len_per_gpu=$(( (max_prompt_length + max_response_length) * 3 ))
log_prob_max_token_len_per_gpu=$(( actor_max_token_len_per_gpu * 3 ))
